...
- from, to:
  ...
  Numbers between 0 and 1 (where `to >= from`) that indicate which part
  of each edge is drawn. By default full lines, so `from` and `to` are
...
- ...:
  ...
  passed on to
  [`tmap::tm_lines()`](https://r-tmap.github.io/tmap/reference/tm_lines.html).
